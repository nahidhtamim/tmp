...
@section('title')
{{$sector->name}} | TMP Manufacturing Ltd
@endsection
    ...
    <div class="container-fluid bg-primary py-5 bg-header">
        <div class="row py-5">
            <div class="col-12 pt-lg-5 mt-lg-5 text-center">
                <h1 class="display-4 text-white animated zoomIn">{{$sector->name}}</h1>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="container py-5">
        <div class="row g-5">
            <div class="col-lg-10 m-auto">
                <p class="mb-4">{!! $sector->description_one !!}</p>
            </div>
            <div class="col-lg-5 m-auto">
                <div class="position-relative h-100">
                    <img class="img-fluid rounded wow zoomIn cover-image" data-wow-delay="0.9s"
                        src="{{ asset('upload/service/'.$sector->image)}}" style="object-fit: cover; border-radius: 20px !important; box-shadow: rgba(0, 0, 0, 0.3) 0px 19px 38px, rgba(0, 0, 0, 0.22) 0px 15px 12px;">
                </div>
            </div>
            @if($sector->description_two != null)
            <div class="col-lg-10 m-auto">
                <p class="mb-4">{!! $sector->description_two !!}</p>
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="container">
        ...
        <div class="row gallery">
            @foreach($galleries as $gallery)
                @if($gallery->service_id == $sector->id)
            ...
            <div class="col-lg-4 wow slideInUp my-4 mx-auto" data-wow-delay="0.3s" style="height: 100%; ">
                <div class="blog-item bg-light rounded overflow-hidden" style=" box-shadow: rgba(0, 0, 0, 0.3) 0px 19px 38px, rgba(0, 0, 0, 0.22) 0px 15px 12px; min-height: 540px;">
                    <div class="blog-img position-relative overflow-hidden">
                        <img class="img-fluid" src="{{ asset('upload/service/'.$service->random)}}" alt="">
                        <a class="position-absolute top-0 start-0 bg-primary text-white rounded-end mt-5 py-2 px-4"
                            href="">{{$random->name}}</a>
                    </div>
                    <div class="p-4">
                        <h4 class="mb-3">{{$random->name}}</h4>
                        <p>{!! substr($random->description_one, 0, 150)."......" !!}</p>
                        <a class="text-uppercase" href="{{url('/our-sectors/'.$random->slug)}}">Find Out More<i class="bi bi-arrow-right"></i></a>
                    </div>
                </div>
            </div>
            {{-- <div class="col-lg-4 wow slideInUp m-auto my-2" data-wow-delay="0.6s" style="box-shadow: rgba(0, 0, 0, 0.3) 0px 19px 38px, rgba(0, 0, 0, 0.22) 0px 15px 12px;">
                <div class="bg-white other-service-details rounded">
                    ...
                    <div class="p-5 pt-0">
                        <p>{!! substr($random->description_one, 0, 150)."......" !!}</p>
                        <a href="{{url('/our-sectors/'.$random->slug)}}" class="btn btn-primary py-2 px-4 mt-4">Find Out More</a>
                    </div>
                </div>
            </div> --}}
            @endforeach
